...
page_header(
    "IT RAGBot - Project AI Assistant",
    "Ask intelligent questions grounded strictly in your uploaded IT project documentation via API endpoints."
)
...
if not project and not documents:
    st.warning("No active IT project found. Please upload a project document first.")
...
with col_info:
    ...
    <div style="background: rgba(15, 23, 42, 0.8); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 12px; padding: 0.75rem 1.2rem; display: flex; align-items: center; gap: 0.75rem;">
        <span style="font-size: 1.2rem;"></span>        <div>
            <div style="font-size: 0.78rem; color: #94a3b8; font-weight: 700; text-transform: uppercase;">Active Project Context</div>
...
if not rag_ready:
    st.warning("Please upload and process an IT document to activate RAGBot.")
    st.stop()
...
with c_head1:
    st.subheader("AI Intelligence Conversation")
with c_head2:
...
if not st.session_state.it_chat_history:
    st.markdown("**Suggested Starter Queries:**")
    starters = [
        "What are the primary technical risks?",
        "Summarize budget and cost overrun status.",
        "Which milestones or deliverables are delayed?",
        "What are the external vendor dependencies?",
        "Provide an executive project health summary."
    ]
...
for message in st.session_state.it_chat_history:
    if message["role"] == "user":
        ...
        with st.chat_message("assistant"):
            st.markdown(
                """
                <div style="background: rgba(15, 23, 42, 0.6); border: 1px solid rgba(56, 189, 248, 0.25); border-radius: 12px; padding: 0.8rem 1.2rem; margin-bottom: 0.8rem; display: flex; align-items: center; gap: 0.6rem;">
                    <span style="font-size: 1.3rem;">🤖</span>
                    <span style="color: #38bdf8; font-weight: 800; font-size: 0.98rem;">AI Project Advisor Answer</span>
                </div>
                """,
                unsafe_allow_html=True
            )
...
question = st.chat_input("Ask a technical question about your project...")

pending = None
if st.session_state.it_chat_history and st.session_state.it_chat_history[-1]["role"] == "user":
    pending = st.session_state.it_chat_history[-1]["content"]
...
if active_question and st.session_state.it_chat_history[-1]["role"] == "user":
    with st.chat_message("assistant"):
        with st.spinner("Searching project vector index & analyzing query context..."):
            try:
                from rag_chatbot.session_store import retrieve
                history_context = st.session_state.it_chat_history[:-1] if len(st.session_state.it_chat_history) > 1 else []
